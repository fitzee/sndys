...
VAR t: LONGINT;
BEGIN
  IF x > 9.0D18 THEN RETURN MAX(LONGINT)
  ELSIF x < -9.0D18 THEN RETURN MIN(LONGINT)
  END;
  IF x >= 0.0D0 THEN
...
VAR q: LONGINT;
BEGIN
  IF y = 0.0D0 THEN RETURN 0.0D0 END;
  IF FAbs(x / y) > 9.0D18 THEN RETURN 0.0D0 END;
...
BEGIN
  ...
  p := 1;
  WHILE p < n DO
    IF p > MAX(CARDINAL) DIV 2 THEN RETURN p END;
    p := p * 2
  END;
  RETURN p
END NextPow2;
